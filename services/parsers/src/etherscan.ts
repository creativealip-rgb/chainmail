/**
 * Etherscan parser.
 * Match: from=*@etherscan.io OR subject contains "Etherscan" / "Tx Hash"
 */
...
export const etherscanParser: Parser = (email: EmailEnvelope): ParsedReceipt | null => {
  const isEtherscan =
    email.from.toLowerCase().includes("etherscan") ||
    email.subject.toLowerCase().includes("etherscan") ||
    email.subject.toLowerCase().includes("tx hash");
  if (!isEtherscan) return null;
